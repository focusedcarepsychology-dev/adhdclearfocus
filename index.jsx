...
async function createCheckout(result,email,isTeen){
  ...
  try{
    if(activeButton){activeButton.innerHTML='⏳ Opening secure checkout...';activeButton.disabled=true;}
  ...
  }catch(e){console.log("Checkout API unavailable",e);}
  finally{if(activeButton){activeButton.innerHTML=originalHtml;activeButton.disabled=false;}}
...
function scoreResults(answers){
  const scores={};let asrsCount=0;let total=0;
  QS.forEach(q=>{
    const val=answers[q.id]??0;
    scores[q.d]=(scores[q.d]||0)+val;
    total+=val;
    if(q.asrs&&val>=2)asrsCount++;
  });
  const pcts={};
  ...
  let level,color;
  if(totalPct>=65||asrsFlag){level="Elevated";color=C.amber;}
  else if(totalPct>=40){level="Moderate";color=C.teal;}
  else{level="Low";color=C.green;}
...
function Bars({active}){
  return(
    <div style={{display:"flex",alignItems:"center",gap:2,height:16}}>
      {[3,6,10,14,10,6,3].map((h,i)=>(
        <div key={i} style={{width:3,height:h,borderRadius:3,
  ...
  return(
    <div style={{marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
        <span style={{fontSize:13,color:C.white,display:"flex",alignItems:"center",gap:6}}>
          <span>{icon}</span><span>{label}</span>
        </span>
        <span style={{fontSize:13,fontWeight:800,color}}>{pct}%</span>
      </div>
      <div style={{background:C.navyMid,borderRadius:99,height:6,overflow:"hidden"}}>
        <div style={{width:`${anim}%`,background:`linear-gradient(to right,${color},${color}CC)`,
...
function FoundingCTA({daysLeft,showDetail,setShowDetail,proEmail,setProEmail,proJoined,onJoin,inPaywall=false}){
  return(
    <div style={{background:"linear-gradient(135deg,#1A0A35,#0A1825)",
      border:"1px solid #A855F760",borderRadius:16,padding:20,marginBottom:inPaywall?0:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
        <div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
            <span style={{fontSize:11,color:"#C084FC",fontWeight:700,textTransform:"uppercase",letterSpacing:1}}>Pro — Founding Member</span>
            <span style={{background:"#A855F720",border:"1px solid #A855F7",borderRadius:99,padding:"2px 8px",
              fontSize:9,color:"#C084FC",fontWeight:700}}>LAUNCHING SOON</span>
          </div>
          <div style={{display:"flex",alignItems:"baseline",gap:12,flexWrap:"wrap"}}>
            <div style={{fontSize:26,fontWeight:900}}>
              €5<span style={{fontSize:12,fontWeight:400,color:C.muted}}>/month</span>
            </div>
            <div style={{background:"#00E67615",border:"1px solid #00E67640",borderRadius:99,
              padding:"2px 10px",fontSize:11,color:C.green,fontWeight:700}}>
              YEARLY · SAVE 50%
            </div>
          </div>
          <div style={{fontSize:11,color:C.muted,marginBottom:2}}>€60/year billed annually · Regular monthly price €10/month</div>
          <div style={{fontSize:11,color:C.green,fontWeight:600}}>
            Assessment completers get this rate locked for life with their discount code
          </div>
        </div>
        <span style={{fontSize:24}}>🚀</span>
      </div>
      {daysLeft>0&&(
        <div style={{background:"#A855F715",border:"1px solid #A855F740",borderRadius:8,
          padding:"8px 12px",marginBottom:12,fontSize:12,color:"#C084FC",textAlign:"center"}}>
          ⏳ Founding member offer closes in <strong>{daysLeft} days</strong>
        </div>
      )}
      <div style={{marginBottom:10}}>
        {PRO_FEATURES.map(f=>(
          <div key={f.title} style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:8}}>
            <span style={{fontSize:14,flexShrink:0}}>{f.icon}</span>
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:1}}>
                <span style={{fontSize:12,fontWeight:600,color:f.status==="available"?C.white:C.muted}}>{f.title}</span>
                <span style={{background:f.status==="available"?C.teal+"25":C.amber+"20",
                  color:f.status==="available"?C.teal:C.amber,fontSize:9,fontWeight:700,
                  padding:"1px 6px",borderRadius:99,textTransform:"uppercase"}}>
                  {f.status==="available"?"Included":"Soon"}
                </span>
              </div>
              {showDetail&&<div style={{fontSize:11,color:C.muted,lineHeight:1.5}}>{f.desc}</div>}
            </div>
          </div>
        ))}
      </div>
      <button onClick={()=>setShowDetail(!showDetail)}
        style={{background:"transparent",border:"none",color:"#C084FC",cursor:"pointer",fontSize:12,marginBottom:12,padding:0}}>
        {showDetail?"Hide details ▲":"Show full details ▼"}
      </button>
      {!proJoined?(
        <div>
          <p style={{fontSize:13,color:C.muted,marginBottom:10,lineHeight:1.55}}>
            Join the founding member waitlist. Your €5/month yearly rate is reserved from sign-up.
            When Pro launches, you'll get a <strong style={{color:C.white}}>7-day free trial — no credit
            card required</strong>. Full access to every Pro feature. Cancel any time and pay nothing.
          </p>
          <div style={{display:"flex",gap:8}}>
            <input type="email" value={proEmail} onChange={e=>setProEmail(e.target.value)}
              onKeyDown={e=>e.key==="Enter"&&onJoin()} placeholder="your@email.com"
              style={{flex:1,background:C.navy,border:"1px solid #A855F760",borderRadius:8,
                padding:"11px 14px",color:C.white,fontSize:14,outline:"none"}}/>
            <button onClick={onJoin}
              style={{background:"#A855F7",color:C.white,border:"none",borderRadius:8,
                padding:"11px 16px",fontWeight:700,fontSize:13,cursor:"pointer",whiteSpace:"nowrap"}}>
              Reserve →
            </button>
          </div>
          <p style={{fontSize:11,color:C.muted,marginTop:8}}>No payment now. Email on launch. Cancel any time.</p>
        </div>
      ):(
        <div style={{background:"#A855F715",border:"1px solid #A855F740",borderRadius:10,
          padding:14,textAlign:"center"}}>
          <div style={{fontSize:18,marginBottom:6}}>🎉</div>
          <div style={{fontWeight:700,fontSize:14,color:"#C084FC",marginBottom:4}}>You are on the list!</div>
          <div style={{color:C.muted,fontSize:12}}>Your founding rate of €5/month (billed yearly) is reserved for life.</div>
        </div>
      )}
    </div>
  );
...
function AuthModal({mode,authEmail,setAuthEmail,authCode,setAuthCode,authError,onRegister,onPro,onClose,daysLeft,C}){
  return React.createElement('div',{
    ...
    React.createElement('div',{style:{background:"#112240",borderRadius:"20px 20px 0 0",
      ...
      React.createElement('div',{style:{width:40,height:4,background:"#1E3A5F",
      ...
      mode==="register"
        ? React.createElement(React.Fragment,null,
            React.createElement('div',{style:{textAlign:"center",marginBottom:16}},
              ...
              React.createElement('div',{style:{fontWeight:900,fontSize:20,marginBottom:6}},"Free account — 10 seconds"),
              React.createElement('div',{style:{fontSize:13,color:"#7B93B4",lineHeight:1.6}},"Unlock all 47 strategies, community, focus rooms and your personal dashboard.")
            ),
            React.createElement('input',{type:"email",value:authEmail,
              onChange:e=>setAuthEmail(e.target.value),
              placeholder:"Your email address",
              onKeyDown:e=>{if(e.key==="Enter")onRegister(authEmail);},
            ...
            authError&&React.createElement('div',{style:{fontSize:12,color:"#FF8080",marginBottom:8}},authError),
            React.createElement('button',{
              ...
              "Get Free Access →"),
            React.createElement('div',{style:{textAlign:"center",fontSize:12,color:"#7B93B4"}},
              "No password · No card · GDPR compliant")
          )
        : React.createElement(React.Fragment,null,
            React.createElement('div',{style:{textAlign:"center",marginBottom:16}},
              React.createElement('div',{style:{fontSize:28,marginBottom:6}},"⭐"),
              React.createElement('div',{style:{fontWeight:900,fontSize:20,marginBottom:6}},"Activate Pro Access"),
              React.createElement('div',{style:{fontSize:13,color:"#7B93B4",lineHeight:1.6}},
                `Enter your loyalty code from your Clarity Report email. Founding rate — ${daysLeft} days left.`)
            ),
            React.createElement('input',{type:"text",value:authCode,
              onChange:e=>setAuthCode(e.target.value.toUpperCase()),
              placeholder:"ACF-XXXXX-XXXX",
              onKeyDown:e=>{if(e.key==="Enter")onPro(authCode);},
            ...
            authError&&React.createElement('div',{style:{fontSize:12,color:"#FF8080",marginBottom:8}},authError),
            React.createElement('button',{onClick:()=>onPro(authCode),
      ...
      React.createElement('button',{onClick:onClose,
        style:{display:"block",width:"100%",background:"transparent",
          ...
          padding:"10px",fontSize:13,cursor:"pointer",fontFamily:"inherit",marginTop:8}},
        "Maybe later")
    )
  );
}
...
function FeedbackWidget({C}){
  ...
  function submit(){
    ...
    try{const ex=JSON.parse(localStorage.getItem("acf_feedback")||"[]");ex.push(entry);localStorage.setItem("acf_feedback",JSON.stringify(ex.slice(-20)));}catch(e){}
    ...
    setSent(true);setTimeout(()=>{setSent(false);setOpen(false);setMsg("");},3000);
  }
  if(sent)return React.createElement('div',{style:{background:"#0A2010",border:"1px solid #00E67640",borderRadius:14,padding:"14px 16px",marginBottom:16,textAlign:"center",fontSize:13,color:"#00E676"}},"✓ Thank you — feedback sent. We read everything.");
  return React.createElement('div',{style:{background:C.navyLight,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px",marginBottom:16}},
    React.createElement('div',{style:{display:"flex",justifyContent:"space-between",alignItems:"center"}},
      React.createElement('div',null,
        React.createElement('div',{style:{fontWeight:700,fontSize:13,marginBottom:2}},"💬 Share feedback"),
        React.createElement('div',{style:{fontSize:11,color:C.muted}},"Suggestions, complaints, questions — we read everything")
      ),
      React.createElement('button',{onClick:()=>setOpen(!open),
        style:{background:open?C.teal:C.navyMid,color:open?C.navy:C.muted,
          ...
          padding:"6px 14px",fontSize:12,cursor:"pointer",fontWeight:700,fontFamily:"inherit"}},
    ...
    open&&React.createElement('div',{style:{marginTop:12}},
      React.createElement('div',{style:{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}},
        ["suggestion","complaint","question","praise"].map(t=>
          React.createElement('button',{key:t,onClick:()=>setType(t),
            style:{background:type===t?C.teal:C.navyMid,color:type===t?C.navy:C.muted,
              ...
              padding:"4px 10px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",textTransform:"capitalize"}},
            t==="suggestion"?"💡 Suggestion":t==="complaint"?"⚠️ Complaint":t==="question"?"❓ Question":"⭐ Praise")
        )
      ),
      React.createElement('textarea',{value:msg,onChange:e=>setMsg(e.target.value),
        ...
        style:{width:"100%",background:C.navy,border:`1px solid ${C.border}`,
          ...
          fontFamily:"inherit",resize:"vertical",minHeight:70,outline:"none",marginBottom:8}}),
      React.createElement('div',{style:{display:"flex",justifyContent:"space-between",alignItems:"center"}},
        React.createElement('span',{style:{fontSize:11,color:C.muted}},
          type==="complaint"?"Responded to within 48 hours":"Anonymous is fine"),
...
function App(){
  ...
  function registerUser(email){
    ...
    localStorage.setItem('acf_email',email);setUserEmail(email);
  ...
  function activatePro(code){
    ...
    if(!/^ACF-[A-Z0-9]{5}-[A-Z0-9]{4}$/.test(clean)){
      setAuthError('Code format: ACF-XXXXX-XXXX — check your report email');return;
    }
    localStorage.setItem('acf_loyalty',clean);localStorage.setItem('acf_pro','true');
    localStorage.setItem('report_purchased','true');
    setIsPro(true);setShowAuthModal(false);setAuthError('');
  ...
  function tr(fn){
    setAnimIn(false);
    setTimeout(()=>{fn();setAnimIn(true);topRef.current?.scrollIntoView({behavior:"smooth"});},200);
  }
  ...
  function handleAnswer(val){
    ...
    setAnswers(newA);
    // Save progress
    try{localStorage.setItem("acf_progress",JSON.stringify({answers:newA,qIndex:qIndex+1,isTeen}));}catch(e){}
    if(qIndex<QS.length-1){tr(()=>setQIndex(qIndex+1));}
    else{
      try{localStorage.removeItem("acf_progress");}catch(e){}
      const r=scoreResults(newA);setResult(r);if(navigator.vibrate)navigator.vibrate([100,50,100,50,200]);tr(()=>setStep("paywall"));
    }
  }
    ...
    try{
      await fetch("/api/mailchimp-subscribe",{method:"POST",
        ...
        body:JSON.stringify({email:emailAddr,tags,
          merge_fields:{ADHD_LEVEL:result?.level||"",ADHD_PCT:String(result?.totalPct||"")}})});
    }catch(e){console.log("Mailchimp:",e);}
  }
  ...
  async function handleEmailSubmit(){
    ...
    if(DISPOSABLE_DOMAINS.includes(domain)){
      setEmailError("Please use a real email address — we need it to send your report.");return;
    }
    setEmailError("");setEmailLoading(true);
    await subscribeToMailchimp(email,["screener-complete",result?.level?.toLowerCase()||""]);
    gtag("event","email_captured",{adhd_level:result?.level});
    setEmailLoading(false);setEmailSubmitted(true);
  }

  async function handleProJoin(){
    if(!proEmail.includes("@")||!proEmail.includes(".")){return;}
  ...
  function restart(){
    try{localStorage.removeItem("acf_progress");}catch(e){}
    setStep("intro");setIsTeen(false);setQIndex(0);setAnswers({});setResult(null);
    setEmail("");setEmailConsent(false);setEmailSubmitted(false);setEmailError("");
    setOpenDomain(null);setProEmail("");setProJoined(false);setShowDetail(false);
    setConsentChecked(false);setShowResume(false);setResumeData(null);
  }

  const fade={opacity:animIn?1:0,transform:animIn?"translateY(0)":"translateY(12px)",
    transition:"opacity 0.2s ease,transform 0.2s ease"};
  const dc=k=>result?.pcts[k]>=65?C.amber:result?.pcts[k]>=40?C.teal:C.green;

  // Domain colour lookup
  const domainColor=k=>DOMAINS[k]?.color||C.teal;
  ...
  return(
    <div style={{background:C.navy,minHeight:"100vh",color:C.white}}>
        ...
        <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:100,
          ...
          <div style={{maxWidth:680,margin:"0 auto"}}>
            <p style={{fontSize:12,color:C.muted,margin:"0 0 10px",lineHeight:1.55}}>
              ADHDclearfocus uses essential cookies only — session management and security.
              No advertising cookies. See our <a href="/legal.html" style={{color:C.teal}}>Privacy Policy</a>.
            </p>
            <div style={{display:"flex",gap:8}}>
      ...
      {/* HEADER */}
      <div style={{background:"rgba(10,22,40,0.95)",backdropFilter:"blur(10px)",
        borderBottom:`1px solid ${C.border}`,padding:"13px 18px",
        display:"flex",alignItems:"center",justifyContent:"space-between",
        position:"sticky",top:0,zIndex:20}}>
        <div style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer"}} onClick={restart}>
          <img src="/logo.png" loading="lazy" aria-hidden="true" alt="Logo" style={{width:28,height:28,borderRadius:"50%",objectFit:"cover"}}/>
          <Bars active={step==="questions"}/>
          <span style={{fontSize:16,fontWeight:900,letterSpacing:"-0.5px"}}>
            ADHD<span style={{color:C.teal}}>clearfocus</span>
          </span>
        </div>
        <a href="/app.html" style={{fontSize:11,color:C.teal,fontWeight:700,
        ...
        {step==="questions"&&(
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{background:C.navyLight,borderRadius:99,height:3,width:80,overflow:"hidden"}}>
              <div style={{width:`${progress}%`,height:"100%",
                background:`linear-gradient(to right,${C.teal},${C.blue})`,
                transition:"width 0.3s ease"}}/>
            </div>
            <span style={{fontSize:11,color:C.muted}}>{qIndex+1}/{QS.length}</span>
          </div>
        )}
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {(step==="paywall"||step==="unlocked")&&(
            <span style={{fontSize:11,color:C.teal,fontWeight:700}}>Profile Complete ✓</span>
          )}
          <button onClick={()=>document.body.classList.toggle("focus-mode")}
            title="Toggle focus mode"
            style={{background:"transparent",border:`1px solid ${C.border}`,
              borderRadius:8,padding:"4px 10px",fontSize:11,color:C.muted,
              cursor:"pointer",display:step==="questions"?"flex":"none",alignItems:"center",gap:4}}>
            🎯 Focus
          </button>
        </div>
      </div>

      <div ref={topRef} style={{maxWidth:700,margin:"0 auto",padding:"0 16px 100px"}}>
        ...
        {step==="intro"&&(
          <div style={fade}>
            ...
            {showResume&&resumeData&&(
              <div style={{background:"linear-gradient(135deg,#0A2535,#0A1628)",
                ...
                <div>
                  ...
                  <div style={{fontSize:12,color:C.muted}}>
                    You completed {resumeData.qIndex} of {QS.length} questions last time.
                  </div>
                </div>
                <div style={{display:"flex",gap:8}}>
                  <button onClick={()=>{
                  ...
                  <button onClick={()=>{
                    ...
                    color:C.muted,borderRadius:8,padding:"8px 12px",fontSize:12,cursor:"pointer"}}>
                    Start fresh
                  </button>
                </div>
              </div>
            ...
            <div style={{
              ...
              <div style={{position:"relative"}}>
                <div style={{display:"inline-flex",alignItems:"center",gap:6,
                  ...
                  fontWeight:700,letterSpacing:1.2,marginBottom:16,textTransform:"uppercase"}}>
                  🧠 The Most Complete ADHD Profile & Personalised Recommendations Available Online
                </div>

                <h1 style={{fontSize:"clamp(24px,6vw,38px)",fontWeight:900,lineHeight:1.15,
                  letterSpacing:"-1.5px",margin:"0 0 16px"}}>
                  The most complete ADHD profile<br/><span style={{
                    background:`linear-gradient(135deg,${C.teal},${C.blue})`,
                    WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>
                    and recommendations that work.</span>
                </h1>

                <p style={{color:"#A8C4D8",fontSize:15,lineHeight:1.7,maxWidth:520,marginBottom:16}}>
                  Most ADHD screeners ask 6 questions and give you a number. ADHDclearfocus measures
                  <strong style={{color:C.white}}> 10 distinct neurological dimensions</strong> — then
                  generates a <strong style={{color:C.teal}}>fully personalised action plan</strong> built
                  around your specific profile. Not generic ADHD advice. Not a one-size-fits-all checklist.
                  A targeted strategy for <em>your</em> brain, your highest-impact dimensions, and your
                  daily life — backed by the strongest available clinical evidence.
                </p>
                <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:20,maxWidth:480}}>
                  {["Your personalised plan targets your specific elevated dimensions — not all ADHD in general",
                    ...
                    <div key={t} style={{display:"flex",gap:8,alignItems:"flex-start"}}>
                      ...
                      <span style={{fontSize:13,color:"#A8C4D8",lineHeight:1.5}}>{t}</span>
                    </div>
                  ))}
                </div>
                ...
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,flexWrap:"wrap"}}>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <div style={{display:"flex",gap:-2}}>
                      {"⭐⭐⭐⭐⭐".split("").map((s,i)=>(
                        <span key={i} style={{fontSize:13}}>{s}</span>
                      ))}
                    </div>
                    <span style={{fontSize:12,color:C.muted}}>Developed by a chartered DPsych psychologist with lived ADHD experience</span>
                  </div>
                </div>

                <button onClick={()=>{gtag("event","screener_start");tr(()=>setStep("consent"));}}
                  style={{background:`linear-gradient(135deg,${C.teal},${C.blue})`,
                    color:C.navy,border:"none",borderRadius:12,padding:"16px 28px",
                    fontSize:17,fontWeight:900,cursor:"pointer",
                    boxShadow:`0 8px 32px ${C.teal}40`,letterSpacing:"-0.3px"}}>
                  Discover Your Full ADHD Profile →
                </button>
                <p style={{fontSize:12,color:C.muted,marginTop:10}}>
                  39 questions · 12–15 minutes · Full 10-dimension profile FREE
                </p>
                <div style={{display:"inline-flex",alignItems:"center",gap:6,
                  ...
                  🎁 7-day free Pro trial after your assessment — no credit card required
                </div>
                <div style={{marginTop:14}}>
                  <a href="/insights.html"
                    onClick={()=>gtag("event","blog_nav_click",{location:"intro"})}
                    style={{color:C.teal,fontSize:13,fontWeight:700,textDecoration:"none"}}>
                    🗂 The Focus Files — the most-searched ADHD topics, answered →
                  </a>
                </div>
                <div style={{marginTop:8}}>
                  <a href="/workplace.html"
                    onClick={()=>gtag("event","employer_nav_click",{location:"intro"})}
                    style={{color:C.amber,fontSize:12,fontWeight:700,textDecoration:"none"}}>
                    🏢 Employer? See workplace programmes →
                  </a>
                </div>
              </div>
            </div>
            ...
            <div style={{marginBottom:20}}>
              <h2 style={{fontSize:16,fontWeight:800,marginBottom:4,textAlign:"center"}}>
                10 Neurological Dimensions Measured
              </h2>
              <p style={{fontSize:13,color:C.muted,textAlign:"center",marginBottom:14,lineHeight:1.5}}>
                Standard ADHD tools measure 2–3 dimensions. ADHDclearfocus measures all 10 areas
                where ADHD affects the brain — giving you a complete neurocognitive map.
              </p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {Object.entries(DOMAINS).map(([key,d])=>(
                  <div key={key} style={{background:C.navyLight,border:`1px solid ${C.border}`,
                    borderRadius:12,padding:"12px 14px",display:"flex",gap:10,alignItems:"flex-start"}}>
                    <span style={{fontSize:18,flexShrink:0}}>{d.icon}</span>
                    <div>
                      <div style={{fontWeight:700,fontSize:12,color:d.color,marginBottom:2}}>{d.label}</div>
                      <div style={{color:C.muted,fontSize:11,lineHeight:1.4}}>{d.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Evidence base */}
            <div style={{background:C.navyLight,border:`1px solid ${C.border}`,
              borderRadius:14,padding:"18px 20px",marginBottom:16}}>
              <div style={{fontSize:13,fontWeight:800,color:C.teal,marginBottom:10,
                textTransform:"uppercase",letterSpacing:0.8}}>
                🔬 Built on the strongest evidence available
              </div>
              <div style={{display:"grid",gap:8}}>
                {[
                  ...
                  <div key={e.fw} style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                    <span style={{color:C.teal,fontSize:12,flexShrink:0,marginTop:2}}>✓</span>
                    <div>
                      <span style={{fontWeight:700,fontSize:12,color:C.white}}>{e.fw}: </span>
                      <span style={{fontSize:12,color:C.muted,lineHeight:1.5}}>{e.desc}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{background:C.navyLight,border:`1px solid ${C.border}`,
              ...
              <div style={{display:"flex",gap:14,alignItems:"flex-start",marginBottom:12}}>
                <img src="/logo_sm.png" alt="ADHDclearfocus"
                  style={{width:52,height:52,borderRadius:"50%",objectFit:"cover",
                    flexShrink:0,border:`2px solid ${C.teal}`}}/>
                <div>
                  <div style={{fontWeight:800,fontSize:14,marginBottom:3}}>Built by experts at the forefront of ADHD understanding</div>
                  <div style={{color:C.muted,fontSize:12,lineHeight:1.65}}>
                    ADHDclearfocus is developed by a multidisciplinary team of chartered psychologists,
                    neurodevelopmental specialists, and researchers with over 15 years of combined clinical
                    experience across NHS, inpatient, and specialist care settings in Ireland and the UK.
                    Our team combines rigorous scientific expertise with lived ADHD experience — because
                    understanding ADHD from the inside changes everything.
                  </div>
                </div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {[
                  ...
                  <div key={i.t} style={{background:C.navy,borderRadius:10,padding:"10px 12px",
                    display:"flex",gap:8,alignItems:"flex-start"}}>
                    <span style={{fontSize:14,flexShrink:0}}>{i.icon}</span>
                    <div>
                      <div style={{fontWeight:700,fontSize:11,color:C.white,marginBottom:2}}>{i.t}</div>
                      <div style={{fontSize:10,color:C.muted,lineHeight:1.4}}>{i.s}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{background:C.navyLight,border:`1px solid ${C.border}`,
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                <div>
                  <div style={{fontSize:11,color:C.purple,fontWeight:700,textTransform:"uppercase",
                    letterSpacing:1,marginBottom:4}}>ADHDclearfocus Pro</div>
                  <div style={{fontWeight:900,fontSize:18}}>
                    Everything you need to manage ADHD daily
                  </div>
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:22,fontWeight:900,color:C.teal}}>€5<span style={{fontSize:12,fontWeight:400,color:C.muted}}>/mo</span></div>
                  <div style={{fontSize:10,color:C.muted}}>yearly · €10 monthly</div>
                </div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
                {[
                  ...
                  <div key={f.t} style={{display:"flex",gap:8,alignItems:"flex-start",
                    ...
                    <div>
                      <div style={{fontWeight:700,fontSize:12,color:C.white,marginBottom:1}}>{f.t}</div>
                      <div style={{fontSize:10,color:C.muted,lineHeight:1.4}}>{f.s}</div>
                    </div>
                  </div>
                ))}
              </div>
              <div style={{background:`${C.teal}10`,border:`1px solid ${C.teal}30`,
                ...
                🎁 <strong style={{color:C.white}}>Complete the assessment today</strong> and receive
                a lifetime discount code for €5/month Pro — locked forever, even when the price rises.
                Plus a <strong style={{color:C.white}}>7-day free trial — no credit card required</strong> when Pro launches.
              </div>
              <button onClick={()=>{gtag("event","screener_start");tr(()=>setStep("consent"));}}
                style={{width:"100%",background:`linear-gradient(135deg,${C.teal},${C.blue})`,
            ...
            {/* Disclaimer */}
            <div style={{background:"#080F1A",border:`1px solid ${C.border}`,
              borderRadius:10,padding:"11px 14px",fontSize:11,color:C.muted,lineHeight:1.65}}>
              <strong style={{color:C.white}}>Important: </strong>
              Educational self-awareness tool only. Results do not constitute a diagnosis of ADHD
              or any other condition and do not establish a clinical relationship.
              If you are in crisis contact the Samaritans free on{" "}
              <strong style={{color:C.white}}>116 123</strong> (Ireland and UK, 24/7).
            </div>
          </div>
        )}
        ...
        {step==="consent"&&(
          <div style={fade}>
            <div style={{padding:"28px 0"}}>
              <h2 style={{fontSize:22,fontWeight:900,marginBottom:8,letterSpacing:"-0.5px"}}>
                Before we begin
              </h2>
              <p style={{color:C.muted,fontSize:14,lineHeight:1.65,marginBottom:20}}>
                ADHDclearfocus measures health-related information under EU GDPR Article 9 
                (special category data). We need your explicit consent before you start.
              </p>

              <div style={{background:C.navyLight,border:`1px solid ${C.border}`,
                borderRadius:14,padding:18,marginBottom:16}}>
                <div style={{fontSize:13,fontWeight:700,marginBottom:12}}>This assessment will:</div>
                {["Collect your responses to 39 questions about ADHD-related experiences",
                  ...
                  "Use anonymised analytics to improve the platform (Google Analytics)"].map(t=>(
              ...
              <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:20,
                ...
                border:`1px solid ${consentChecked?C.teal:C.border}`,cursor:"pointer"}}
                onClick={()=>setConsentChecked(!consentChecked)}>
                <div style={{width:20,height:20,borderRadius:5,border:`2px solid ${consentChecked?C.teal:C.muted}`,
                  background:consentChecked?C.teal:"transparent",flexShrink:0,
                  display:"flex",alignItems:"center",justifyContent:"center",marginTop:1}}>
                  {consentChecked&&<span style={{color:C.navy,fontSize:12,fontWeight:900}}>✓</span>}
                </div>
                <div style={{fontSize:13,color:C.muted,lineHeight:1.6}}>
                  I consent to ADHDclearfocus processing my responses as health-related data
                  to generate my personalised ADHD profile. I understand this is an educational
                  tool, not a clinical diagnosis. I have read the{" "}
                  <a href="/legal.html" style={{color:C.teal}}>Privacy Policy</a>.
                </div>
              </div>

              <button onClick={()=>{if(consentChecked)tr(()=>setStep("age"));}}
                style={{width:"100%",
                  ...
                  transition:"all 0.2s"}}>
                {consentChecked?"Continue to Assessment →":"Please tick the consent box above"}
              </button>
            </div>
          </div>
        )}
        ...
        {step==="age"&&(
          <div style={fade}>
            <div style={{padding:"28px 0"}}>
              <h2 style={{fontSize:22,fontWeight:900,marginBottom:6}}>One quick question</h2>
              <p style={{color:C.muted,marginBottom:22,fontSize:14,lineHeight:1.6}}>
                This helps us use age-appropriate language throughout your assessment.
              </p>
              <div style={{display:"grid",gap:10}}>
                {[{l:"I am 13–17 years old",s:"Adolescent version — school and home context",teen:true},
                  {l:"I am 18 or older",s:"Adult version — work, relationships and daily life",teen:false}].map(o=>(
                  <button key={o.l} onClick={()=>{setIsTeen(o.teen);tr(()=>setStep("questions"));}}
                    style={{background:C.navyLight,border:`2px solid ${C.border}`,borderRadius:14,
                    ...
                    <div>
                      <div style={{fontWeight:700,fontSize:15,marginBottom:4}}>{o.l}</div>
                      <div style={{color:C.muted,fontSize:12}}>{o.s}</div>
                    </div>
          ...
          <div style={fade}>
            <div style={{paddingTop:20}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:14}}>{DOMAINS[currentQ.d]?.icon}</span>
                  <span style={{fontSize:11,color:DOMAINS[currentQ.d]?.color||C.teal,
                    fontWeight:700,textTransform:"uppercase",letterSpacing:1}}>
                    {DOMAINS[currentQ.d]?.label}
                  </span>
                </div>
              ...
              <div style={{background:`linear-gradient(135deg,${C.navyLight},${C.navyMid})`,
                border:`1px solid ${DOMAINS[currentQ.d]?.color||C.teal}40`,
                borderRadius:16,padding:"24px 20px",marginBottom:16,
                boxShadow:`0 4px 24px ${DOMAINS[currentQ.d]?.color||C.teal}10`}}>
                <p style={{fontSize:"clamp(15px,3.5vw,18px)",fontWeight:600,lineHeight:1.6,margin:0}}>
                  {isTeen?currentQ.teen:currentQ.a}
                </p>
              </div>

              {/* Answer options */}
              <div style={{display:"grid",gap:8}}>
                {(currentQ.t==="agree"?AGREE:FREQ).map(opt=>{
                  const sel=answers[currentQ.id]===opt.v;
                  const col=DOMAINS[currentQ.d]?.color||C.teal;
                  return(
                    <button key={opt.v} onClick={()=>{if(navigator.vibrate)navigator.vibrate(25);handleAnswer(opt.v);}}
                      style={{background:sel?col:C.navyLight,
                        ...
                        alignItems:"center"}}>
                      <span>{opt.l}</span>
                      {sel&&<span style={{fontWeight:900}}>✓</span>}
                    </button>
                  );
                })}
              </div>
              ...
              {qIndex>0&&(
                <button onClick={()=>tr(()=>setQIndex(qIndex-1))}
                  style={{background:"transparent",border:"none",color:C.muted,
                    cursor:"pointer",fontSize:12,marginTop:14,padding:"4px 0"}}>
                  ← Previous question
                </button>
              )}
            </div>
        ...
        {step==="paywall"&&result&&(
          <div style={fade}>
            <div style={{paddingTop:20,animation:"slideInUp 0.5s ease"}}>
              {/* Result card */}
              <div style={{
                ...
                <div style={{display:"inline-block",
                  ...
                  color:result.color,letterSpacing:1.2,textTransform:"uppercase",marginBottom:12,animation:"slideInUp 0.5s ease"}}>
                  ✓ Your 10-Dimension Profile is Ready
                </div>
                <h2 style={{fontSize:"clamp(17px,4vw,24px)",fontWeight:900,lineHeight:1.3,
                  margin:"0 0 10px",letterSpacing:"-0.5px"}}>
                  {result.level==="Elevated"?"Your profile shows a significant ADHD neurological pattern":
              ...
              {/* ASRS flag — show teaser only, not details */}
              {result.asrsFlag&&(
                <div style={{background:"#1A1000",border:"1px solid #FFB34780",
                  borderRadius:12,padding:"14px 16px",marginBottom:16,display:"flex",gap:12}}>
                  <span style={{fontSize:18,flexShrink:0}}>⚠️</span>
                  <div>
                    <div style={{fontWeight:700,color:C.amber,fontSize:13,marginBottom:3}}>
                      Clinical screening threshold triggered
                    </div>
                    <div style={{color:C.muted,fontSize:12,lineHeight:1.55}}>
                      A validated clinical screening threshold was met in your responses.
                      The full clinical interpretation, what this means for you specifically,
                      and your recommended next steps are included in your Clarity Report.
                    </div>
                  </div>
                </div>
              )}

              {/* Social proof + population context */}
              <div style={{background:C.navyLight,border:`1px solid ${C.border}`,
                ...
                <div style={{display:"flex",justifyContent:"space-between",
                  ...
                  {[
                    ...
                    <div key={s.l} style={{textAlign:"center",flex:1,minWidth:80}}>
                      <div style={{fontSize:18,fontWeight:900,color:C.teal}}>{s.n}</div>
              ...
              {/* Locked dimensions preview */}
              <div style={{background:C.navyLight,border:`1px solid ${C.border}`,
                borderRadius:14,padding:"16px",marginBottom:16}}>
                <div style={{fontSize:13,fontWeight:700,marginBottom:8,color:C.muted}}>
                  🔒 Your personalised plan is locked
                </div>
                <div style={{fontSize:12,color:C.muted,marginBottom:12,lineHeight:1.55}}>
                  Your report contains a <strong style={{color:C.white}}>targeted strategy plan built
                  specifically for your neurological profile</strong> — prioritised by your highest-impact
                  dimensions, with step-by-step evidence-based actions. Not generic ADHD advice.
                  Your plan, for your brain.
                </div>
                <div style={{display:"grid",gap:8}}>
                  {Object.entries(DOMAINS).map(([key,d])=>(
                    <div key={key} style={{display:"flex",justifyContent:"space-between",
                      ...
                      <div style={{background:C.navyMid,borderRadius:6,padding:"2px 10px",
                        fontSize:12,color:C.muted}}>🔒 Locked</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Email capture */}
              {!emailSubmitted?(
                <div style={{background:C.navyLight,border:`1px solid ${C.teal}40`,
                  borderRadius:16,padding:"20px",marginBottom:16}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                    <div style={{width:8,height:8,borderRadius:"50%",background:C.teal,
                      animation:"pulse 2s infinite"}}/>
                    <div style={{fontWeight:800,fontSize:15}}>Your profile is ready</div>
                  </div>
                  <div style={{color:C.muted,fontSize:13,marginBottom:14,lineHeight:1.6}}>
                    Enter your email to unlock your complete 10-dimension profile and access your personalised report.
                  </div>
                  <div style={{display:"flex",gap:8,marginBottom:8}}>
                    <input type="email" value={email}
                      ...
                      style={{flex:1,background:C.navy,
                    ...
                    <button onClick={handleEmailSubmit} disabled={emailLoading}
                      style={{background:emailLoading?"#333":`linear-gradient(135deg,${C.teal},${C.blue})`,
                        color:C.navy,border:"none",borderRadius:10,padding:"13px 18px",
                        fontWeight:800,fontSize:14,cursor:emailLoading?"not-allowed":"pointer",minWidth:90}}>
                      {emailLoading?"...":"Unlock →"}
                    </button>
                  </div>
                  {emailError&&<div style={{fontSize:12,color:C.red,marginBottom:8}}>{emailError}</div>}
                  <label style={{display:"flex",alignItems:"flex-start",gap:8,cursor:"pointer"}}>
                    <input type="checkbox" checked={emailConsent}
                      onChange={e=>setEmailConsent(e.target.checked)}
                      style={{marginTop:3,flexShrink:0,accentColor:C.teal}}/>
                    <span style={{fontSize:11,color:C.muted,lineHeight:1.55}}>
                      I agree to receive my results and evidence-based ADHD content from ADHDclearfocus.
                ...
                <div>
                  {/* FREE full 10-dimension profile */}
                  {result.asrsFlag&&(
                    <div style={{background:"#1A1000",border:"1px solid #FFB34780",
                      ...
                      <div>
                        <div style={{fontWeight:700,color:C.amber,fontSize:13,marginBottom:3}}>
                          WHO ASRS-v1.1: Positive ({result.asrsCount}/6 Part A items)
                        </div>
                        <div style={{color:C.muted,fontSize:12,lineHeight:1.55}}>
                          Sensitivity 68.7%, specificity 99.5% (Kessler et al., 2005).
                          Formal assessment with a qualified clinician is recommended.
                        </div>
                      </div>
                    </div>
                  )}
                  <div style={{background:C.navyLight,border:`1px solid ${C.teal}50`,
                    borderRadius:16,padding:"18px 20px",marginBottom:12}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                      <div style={{fontWeight:800,fontSize:15}}>Your 10-Dimension Profile</div>
                      <div style={{background:"#00E67615",border:"1px solid #00E67640",borderRadius:99,
                        padding:"2px 10px",fontSize:10,color:C.green,fontWeight:800,letterSpacing:1}}>FREE</div>
                    </div>
                    {Object.keys(DOMAINS).map(key=>{
                      const d=DOMAINS[key];
                      return <DBar key={key} label={d.label} icon={d.icon}
                        pct={result.pcts[key]||0} color={dc(key)}/>;
                    })}
                    <div style={{fontSize:11,color:C.muted,marginTop:10,lineHeight:1.55}}>
                      This profile is educational, not diagnostic. Your personalised interpretation —
                      what your specific pattern means and exactly what to do about it — is in the Clarity Report below.
                    </div>
                  </div>

                  {/* Report CTA */}
                  <div style={{background:C.navyLight,border:`2px solid ${C.teal}`,
                    ...
                    <div style={{position:"absolute",top:-12,right:16,
                      ...
                      padding:"3px 12px",borderRadius:99}}>AVAILABLE NOW</div>
                    <div style={{display:"flex",justifyContent:"space-between",
                      alignItems:"flex-start",marginBottom:12}}>
                      <div>
                        <div style={{fontSize:11,color:C.teal,fontWeight:700,
                          textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>
                          Go Deeper — Clarity Report
                        </div>
                        <div style={{fontSize:30,fontWeight:900}}>
                          €49 <span style={{fontSize:13,fontWeight:400,color:C.muted}}>one-off</span>
                        </div>
                      </div>
                    ...
                    {["AI-personalised interpretation of YOUR exact pattern",
                      ...
                      "Delivered to your inbox within 24 hours"].map(f=>(
                  ...
                  <FoundingCTA daysLeft={daysLeft} showDetail={showDetail}
                    setShowDetail={setShowDetail} proEmail={proEmail}
                    setProEmail={setProEmail} proJoined={proJoined}
                    onJoin={handleProJoin} inPaywall={true}/>

                  <div style={{display:"flex",justifyContent:"center",gap:16,marginTop:12,flexWrap:"wrap"}}>
                    {["🔒 Secure checkout","📧 Delivered within 24 hours","↩ 7-day refund policy"].map(s=>(
                      <span key={s} style={{fontSize:11,color:C.muted}}>{s}</span>
                    ))}
                  </div>

                  {/* 15% discount on clinical assessments */}
                  <div style={{background:"linear-gradient(135deg,#0A1A10,#0A1628)",
                    border:"1px solid #00E67640",borderRadius:14,padding:"16px",marginTop:12}}>
                    <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:8}}>
                      <span style={{fontSize:18,flexShrink:0}}>🏥</span>
                      <div>
                        <div style={{fontWeight:800,fontSize:13,color:C.green,marginBottom:3}}>
                          15% Off Formal ADHD & ASD Assessments
                        </div>
                        <div style={{fontSize:12,color:C.muted,lineHeight:1.6}}>
                          ADHDclearfocus subscribers receive a <strong style={{color:C.white}}>15% discount
                          on formal ADHD and ASD clinical assessments</strong> through our registered clinical
                          partners in Ireland and the UK. Your assessment code is generated automatically
                          on subscription.
                        </div>
                      </div>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}>
                      {[
                        ...
                        <div key={i.t} style={{background:"#00E67608",border:"1px solid #00E67625",
                          borderRadius:8,padding:"8px 10px",display:"flex",gap:6,alignItems:"flex-start"}}>
                          <span style={{fontSize:12,flexShrink:0}}>{i.icon}</span>
                          <div>
                            <div style={{fontSize:11,fontWeight:700,color:C.white}}>{i.t}</div>
                            <div style={{fontSize:10,color:C.muted}}>{i.s}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                    <div style={{background:"#00E67610",border:"1px solid #00E67630",
                      borderRadius:8,padding:"10px 12px",fontSize:11,color:C.muted,lineHeight:1.6}}>
                      <strong style={{color:C.green}}>How it works:</strong> Subscribe to any ADHDclearfocus
                      plan to receive your unique 15% discount code. Valid with our registered clinical
                      partners who meet DSM-5/ICD-11 diagnostic standards — the same standards accepted
                      by HSE, NHS, and government services. Partners listed in your member dashboard on launch.
                    </div>
                  </div>

                  {/* Professional support — Psychology Today */}
                  <div style={{background:C.navyLight,border:`1px solid ${C.border}`,
                    ...
                    <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                      ...
                      <div>
                        ...
                        <div style={{fontSize:12,color:C.muted,lineHeight:1.6,marginBottom:10}}>
                          A screener is a starting point, not a diagnosis. If your profile raises
                          questions you want to talk through, you can speak with a chartered
                          psychologist experienced in adult ADHD — a free 15-minute initial
                          consultation is available.
                        </div>
                        <a href="https://www.psychologytoday.com/ie/counselling/conall-donegan-waterford-wd/1437075"
                          target="_blank" rel="noopener"
                          onClick={()=>gtag("event","pt_profile_click",{location:"paywall"})}
                          style={{display:"inline-block",background:C.navyMid,border:`1px solid ${C.teal}60`,
                          color:C.teal,borderRadius:8,padding:"9px 14px",fontSize:12,fontWeight:700,
                          textDecoration:"none"}}>
                          Speak to a psychologist (Psychology Today) →
                        </a>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{background:"linear-gradient(135deg,#1A1206,#0A1628)",
                    border:"1px solid #FFB34740",borderRadius:14,padding:"16px",marginTop:12}}>
                    <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                      <span style={{fontSize:18,flexShrink:0}}>📓</span>
                      <div>
                        <div style={{fontSize:10,color:C.amber,fontWeight:700,
                        ...
                        <div style={{fontSize:12,color:C.muted,lineHeight:1.6,marginBottom:10}}>
                          A structured, evidence-based 90-day gratitude programme written by a
                          DPsych psychologist. Gratitude practice activates the prefrontal cortex —
                          the region underactivated in ADHD — and it takes seconds per day, designed
                          for people who find habits difficult.
                        </div>
                        <a href="https://www.amazon.co.uk/Gratitude-Prescription-Doctors-Program-Seconds/dp/B0GPBYKYYL"
                          target="_blank" rel="noopener"
                          onClick={()=>gtag("event","book_click",{location:"paywall"})}
                          style={{display:"inline-block",background:C.amber,color:C.navy,
            ...
            <div style={{paddingTop:20}}>
              ...
              <div style={{
                ...
                <div style={{display:"inline-block",
                  ...
                  color:result.color,letterSpacing:1.2,textTransform:"uppercase",marginBottom:10}}>
                  ✓ Your 10-Dimension Profile is Ready
                </div>
                <p style={{color:C.muted,fontSize:13,lineHeight:1.6}}>
                  Your complete 10-dimension profile is below. Your personalised PDF report
              ...
              <div style={{background:C.navyLight,border:`1px solid ${C.border}`,
                ...
                <div style={{fontWeight:800,fontSize:15,marginBottom:16}}>
                  Your 10-Dimension Neurological Profile
                </div>
                {Object.entries(DOMAINS).map(([key,d])=>(
                  <DBar key={key} label={d.label} icon={d.icon}
                    pct={result.pcts[key]||0} color={dc(key)}/>
                ))}
                <div style={{fontSize:11,color:C.muted,marginTop:10,lineHeight:1.5}}>
                  🟡 Amber (65%+) = Clinically significant ·
                  🔵 Teal (40–64%) = Moderate ·
              ...
              <div style={{marginBottom:16}}>
                ...
                <div style={{color:C.muted,fontSize:12,marginBottom:12,lineHeight:1.5}}>
                  Tap any dimension to expand your personalised strategies.
                  All strategies cite peer-reviewed clinical literature.
                </div>
                {Object.entries(DOMAINS).map(([key,d])=>{
                  ...
                  const items=pct>=50?(recs?.high||[]):(recs?.mod||[]);
                  if(!items.length)return null;

                  return(
                    <div key={key} style={{background:C.navyLight,border:`1px solid ${C.border}`,
                      borderRadius:13,marginBottom:8,overflow:"hidden"}}>
                      <button onClick={()=>setOpenDomain(isOpen?null:key)}
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          ...
                          <div>
                            ...
                            <div style={{color:C.muted,fontSize:11,marginTop:1}}>
                              {result.pcts[key]}% · {pct>=65?"Priority":"Focus"} area
                            </div>
                          </div>
                        </div>
                        <span style={{color:C.muted,transform:isOpen?"rotate(180deg)":"rotate(0deg)",
                          transition:"transform 0.2s",display:"inline-block",fontSize:16}}>▾</span>
                      </button>
                      {isOpen&&(
                        <div style={{padding:"0 16px 16px",borderTop:`1px solid ${C.border}`}}>
                          {items.map((s,i)=>(
                            <div key={i} style={{display:"flex",gap:10,background:C.navy,
                              borderRadius:10,padding:"12px 14px",marginBottom:7,marginTop:i===0?12:0}}>
                              <span style={{color,fontWeight:800,fontSize:13,flexShrink:0,marginTop:1}}>
                                {i+1}.
                              </span>
                              <p style={{color:C.muted,fontSize:12,lineHeight:1.7,margin:0}}>{s}</p>
                            </div>
                          ))}
                        </div>
                      )}
              ...
              {/* Strategies link */}
              <a href="/strategies.html" style={{textDecoration:"none"}}>
                <div style={{background:"linear-gradient(135deg,#0A1528,#112240)",
                  ...
                  <div>
                    <div style={{fontWeight:800,fontSize:14,marginBottom:3}}>
                      47 Evidence-Based Coping Strategies
                    </div>
                    <div style={{fontSize:12,color:C.muted,lineHeight:1.55}}>
                      Full animated strategy guide — includes a built-in focus timer, mood check-in,
                      and step-by-step guides for every ADHD dimension.
                    </div>
                  </div>
              ...
              <div style={{background:C.navyLight,border:`1px solid ${C.border}`,
                ...
                <div style={{fontSize:11,color:C.amber,fontWeight:700,textTransform:"uppercase",
                  letterSpacing:1,marginBottom:10}}>🚀 Coming in Pro</div>
                <div style={{display:"grid",gap:8}}>
                  {[
                    ...
                    <div key={f.title} style={{display:"flex",gap:10,alignItems:"flex-start",
                      ...
                      <span style={{fontSize:16,flexShrink:0}}>{f.icon}</span>
                      <div>
                        <div style={{fontWeight:700,fontSize:12,color:C.white,marginBottom:2}}>{f.title}</div>
              ...
              <div style={{
                ...
                <div style={{fontWeight:900,fontSize:15,marginBottom:6}}>
                  Your 14-page PDF report is on its way
                </div>
                <div style={{color:C.muted,fontSize:13,lineHeight:1.6,marginBottom:12}}>
                  A personalised evidence-based report covering all 10 neurological dimensions
                  has been sent to your email.
                </div>
                <div style={{background:C.navy,border:`1px solid ${C.border}`,borderRadius:8,
                  padding:"8px 16px",display:"inline-block",fontSize:12,color:C.teal}}>
                  📧 Check your inbox
                </div>
              </div>


              {/* Loyalty discount — assessment completers */}
              <div style={{background:"linear-gradient(135deg,#0A2010,#0A1628)",
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                  <div>
                    ...
                    <div style={{fontSize:11,color:C.green,fontWeight:700,
                      ...
                      🎁 Assessment Completer Reward
                    </div>
                    <div style={{background:"#FF525220",border:"1px solid #FF525240",
                      borderRadius:99,padding:"2px 8px",fontSize:10,color:"#FF8080",fontWeight:700}}>
                      {daysLeft} days left · Founding rate
                    </div>
                  </div>
                    <div style={{fontSize:22,fontWeight:900,marginBottom:2}}>
                      €5<span style={{fontSize:12,fontWeight:400,color:C.muted}}>/month for life</span>
                    </div>
                    <div style={{fontSize:11,color:C.muted}}>Regular Pro price: €10/month · Your rate: locked forever</div>
                  </div>
                ...
                <div style={{background:`${C.green}10`,border:`1px solid ${C.green}30`,
                  ...
                  <div style={{fontSize:11,color:C.muted,marginBottom:4}}>Your lifetime discount code</div>
                  <div style={{fontSize:20,fontWeight:900,color:C.green,letterSpacing:4,fontFamily:"monospace"}}>
                    ACF-{email.split("@")[0].toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,6)||"EARLY"}-50
                  </div>
                  <div style={{fontSize:11,color:C.muted,marginTop:4}}>
                    Use this code when Pro launches to lock your €5/month rate permanently.
                    If you cancel and rejoin without the code, the rate will be €10/month.
                  </div>
                </div>
                <div style={{fontSize:12,color:C.muted,lineHeight:1.6}}>
                  This code is tied to your email address and valid for life. 
                  Save it now — it will also be included in your report email.
                </div>
              </div>
              ...
              <FoundingCTA daysLeft={daysLeft} showDetail={showDetail}
                setShowDetail={setShowDetail} proEmail={proEmail}
                setProEmail={setProEmail} proJoined={proJoined} onJoin={handleProJoin}/>

              {/* Review call */}
              <div style={{background:"#1A0A35",border:"1px solid #A855F760",
                borderRadius:16,padding:"18px",marginBottom:16}}>
                <div style={{fontWeight:900,fontSize:14,marginBottom:6}}>
                  🎯 Want to go deeper with a specialist?
                </div>
                <div style={{color:C.muted,fontSize:13,lineHeight:1.65,marginBottom:12}}>
                  Book a 30-minute report review call with our clinical psychologist. Walk through
                  your 10-dimension profile, get specific answers to your questions, and leave
                  with a clear, personalised action plan.
                </div>
                <a href={STRIPE_LINKS.reviewCall} target="_blank" rel="noopener noreferrer"
                  style={{textDecoration:"none"}}>
                  <button style={{background:"#A855F7",color:C.white,border:"none",borderRadius:10,
                    padding:"12px 20px",fontWeight:700,fontSize:13,cursor:"pointer"}}>
                    Book 30-min Review Call — €79
                  </button>
                </a>
              </div>
              ...
              <div style={{background:"#080F1A",border:`1px solid ${C.border}`,
                ...
                <strong style={{color:C.white}}>Samaritans 116 123</strong>
              ...
              <div style={{background:C.navyLight,border:`1px solid ${C.border}`,
                ...
                <div style={{fontSize:12,color:C.muted,marginBottom:10,lineHeight:1.55}}>
                  Share your ADHD profile level — it might help someone you know recognise their own patterns.
                </div>
                <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap"}}>
                  {[
                    ...
                    <button key={s.label} onClick={()=>{
                      ...
                      else{navigator.clipboard?.writeText("https://adhdclearfocus.ie");
                        alert("Link copied!");}
